...
# mgl: cloned this from aldex.r version 2.0.7
#       refactored a couple of the lapply and loops for performance to be SMP aware
aldexSMP <- function( reads, conditions, mc.samples=128, paired.test=FALSE, gene.ratios=FALSE, ratio.method="", verbose=TRUE) {
    # make sure that the multicore package is in scope
    require(multicore)
...
###########
#
# If examining antisense RNA, coerce data for gene ratio analysis into interleaved row format
# tidied up this code to put everything inside one if statement gg, Apr 1, 2014
#
###########	BEGIN GENE RATIO COERCION
	#JMD: if gene.ratios==TRUE ratio.method must be "columns" or "rows"
	if (gene.ratios==TRUE) {
		if (ratio.method != "columns" | ratio.method != "rows") {
		...
		if (ratio.method=="columns") {
			...
			if (ratio.method=="rows" && nrow(reads)%%2 != 0) {
...
######WILL NEED TO CHANGE THIS FOR MORE COMPLICATED DESIGNS
...
#######END CHANGES FOR MORE COMPLICATED DESIGNS

	if ( length( setA ) < 3 ) warning("t-test requires at least 3 replicates in set A, use effect for significance and at least 1028 DMC")
    if ( length( setB ) < 3 ) warning("t-test requires at least 3 replicates in set B, use effect for significance and at least 1028 DMC")
...
# change to a multicore paradigm
    p <- mclapply(
        reads , function(col) {
            q <- t( rdirichlet( mc.samples, col + 0.5 ) ) ; rownames(q) <- rn ; q
...
########## 
#
#     Hook into l2p for more complex data sets using the data in l2p
#     Will require altering the data sanity checks above
#     Will require altering the summary statistics below
#
#########
...
# break the MC looping code out to its own function because we want to run it in parallel
anMCinstance <- function(instance,geneRatios=FALSE)
{
	#make the data set for each monte carlo realization
	t.input.data <- matrix(data=NA, nrow=nrow(reads), ncol=(length(l2p)))
	for(i in 1:length(l2p)){ 
		t.input.data[,i] <- l2p[[i]][,instance] 
	}
	
	###JMD: if gene.ratios==TRUE calculate them here
	###MGL: note that this is now moved into a function and so gene.ratios is being passed to the argument geneRatios
  	if (geneRatios==TRUE) {
	...
	#do the t test
	#MGL: this is significantly changed because we need to pass the results back by value
	#     because in a parallel environment, and we don't have a semophore

	# Generate the wilcox results - uncorrected for multiple testing  
	wilcoxResult <- t(apply(t.input.data, 1, function(t.input.data){as.numeric(wilcox.test(x=t.input.data[setA],y=t.input.data[setB])[3:4])}))

	# Generate the welch's results - uncorrected for multiple testing
	welchsResult <- t(apply(t.input.data, 1, function(t.input.data){as.numeric(t.test(x=t.input.data[setA],y=t.input.data[setB], paired=paired.test)[1:3])}))
   
	# reformulate the results and add the multiple testing correction
	# then prepare a list of these elements to return
   
	ret <- list(
		"wilcoxP"       = as.numeric(wilcoxResult[,1]),
		"wilcoxTBH"     = as.numeric(p.adjust(wilcoxResult[,1], method="BH")),
		"welchsP"       = welchsResult[,3],
		"welchsTBH"     = as.numeric(p.adjust(welchsResult[,3], method="BH"))
	)
	return(ret)
}

#MGL: use the multicore lapply paradigm here to get everything done in parallel
resultSet <- mclapply(1:mc.samples,FUN=anMCinstance,geneRatios=gene.ratios,mc.cores=multicore:::detectCores())

#MGL: a really stoopid for loop to put the values of results back into the wi and we matricies
# there is probably a much more R-centric way to do this but that's an exercise to the reader
# in my hands this doesn't seem to be taking up much wall-time, I have done no profiling for memory usage
for(i in 1:mc.samples){
        wi.p.matrix[,i]         <- resultSet[[i]]$wilcoxP
        wi.tBH.matrix[,i]       <- resultSet[[i]]$wilcoxTBH
        we.p.matrix[,i]         <- resultSet[[i]]$welchsP
        we.tBH.matrix[,i]       <- resultSet[[i]]$welchsTBH
}
...
# change to multicore paradigm
    rab$spl <- mclapply( l2p, function(m) { t(apply( m, 1, quantile, probs=probs, type=8 )) }, mc.cores=multicore:::detectCores())
...
# apply multicore paradigm
    l2d$win  <- mclapply( l2d$win, function(arg) { arg[,1:ncol.wanted] },mc.cores=multicore:::detectCores() )
...
#make and fill in the final data table
    rv <- list(
        rab = rab,
        diff = l2s,
        effect = effect,
...
plot.aldex <- function( x, ..., type=c("MW","MA","hist"),
    ...
    signif.col="cyan", signif.pch=20, signif.cex=0.6,
    ...
    if (test == "effect"){
    	if (cutoff < 1) {stop("must set effect size to greater than 1 for effect calculation")}
     	called <- (abs(x$effect[,"50%"]) >= cutoff & x$overlap < 0.01)
      	signif <- x$effect[,"50%"] >= cutoff
  	}
