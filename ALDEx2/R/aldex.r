aldex <- function(reads, conditions, mc.samples=128, test="t", effect=TRUE, include.sample.summary=FALSE, verbose=FALSE, input="default"){
    
    # wrapper function for the entire set of 
    print("aldex.clr: generating Monte-Carlo instances and clr values")
    x <- aldex.clr(reads=reads, conds=conditions, mc.samples=mc.samples, input=input, verbose=verbose, useMC=FALSE)
    if(test == "t") {
    ...
    if(effect == TRUE){
        ...
        x.effect <- aldex.effect(x, conditions, include.sample.summary=include.sample.summary, verbose=verbose)
